...
class RabbitQueueManager {
    ...
    void openListener(RabbitListenerRequest request) {
        ...
        if (request.exclusiveConnectionFlag) {
            ...
            def connectionSuccessHandler = ({ Connection connection ->
                // declare the queue(s)
                def listenerResult = this._declareQueuesOnConnection(connection, request)
     ...
     * @param request
     */
    private RabbitListenerResult _declareQueuesOnConnection(Connection conn, RabbitListenerRequest request) {
